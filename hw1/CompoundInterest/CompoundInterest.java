...
public class CompoundInterest {
    ...
    static double futureValueReal(double presentValue, double rate,
        ...
        return presentValue * Math.pow(((rate/100)+1),(numYears(targetYear)))*Math.pow((1-inflationRate/100),numYears(targetYear));
    }
    ...
    static double totalSavings(double perYear, int targetYear, double rate) {
        int temp = numYears(targetYear);
        double total = 0;
        while (temp >= 0){
            total += perYear * Math.pow((rate/100 + 1), temp);
            temp -= 1;
        }
        return total;
    }
    ...
    static double totalSavingsReal(double perYear, int targetYear, double rate,
        ...
        double ts = totalSavings(perYear,targetYear,rate);
        return ts * Math.pow(1-(inflationRate/100), numYears(targetYear));
    }
