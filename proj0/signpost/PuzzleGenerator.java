    ...
    private int makeForwardConnections(Model model) {
        ...
        for (Sq sq : model) {
            ...
            if (sq.successor() == null && sq.direction() != 0) {
                PlaceList sqs1 = _successorCells[sq.x][sq.y][sq.direction()];
                PlaceList sqs = new PlaceList();
                for (int i = 0; i < sqs1.size(); i++) {
                    if (sq.connectable(model.get(sqs1.get(i).x,
                        sqs1.get(i).y))) {
                        nFound += 1;
                        sqs.add(sqs1.get(i));
                        found = model.get(sqs1.get(i).x,
                            sqs1.get(i).y);
                    }
                }
                nFound = sqs.size();
                if (nFound == 0) {
                    return 0;
                } else if (nFound == 1 && sq.sequenceNum() != 0) {
                    found = model.get(sqs.get(0));
                    sq.connect(found);
                    result = 2;
                } else {
                    int randomNum = _random.nextInt(nFound);
                    found = model.get(sqs.get(randomNum).x,
                        sqs.get(randomNum).y);
                    if (sq.sequenceNum() != 0) {
                        for (Place s : sqs) {
                            if (model.get(s).sequenceNum() != 0) {
                                if (sq.connectable(model.get(s))) {
                                    nFound = 1;
                                    found = model.get(s);
                                }
                            }
    ...
    private int makeBackwardConnections(Model model) {
        ...
        for (Sq sq : model) {
            ...
            if (sq.predecessor() == null && sq.sequenceNum() != 1) {
                PlaceList psqs1 = sq.predecessors();
                PlaceList psqs = new PlaceList();
                for (int i = 0; i < psqs1.size(); i++) {
                    if (model.get(psqs1.get(i).x,
                        psqs1.get(i).y).connectable(sq)) {
                        nFound += 1;
                        psqs.add(psqs1.get(i));
                        found = model.get(psqs1.get(i).x,
                            psqs1.get(i).y);
                    }
                }
                nFound = psqs.size();
                if (nFound == 0) {
                    ...
                    found = model.get(psqs.get(0));
                    found.connect(sq);
                    result = 2;
                } else {
                    int randomNum = _random.nextInt(nFound);
                    found = model.get(psqs.get(randomNum).x,
                        psqs.get(randomNum).y);
                    if (sq.sequenceNum() != 0) {
                        for (Place s : psqs) {
                            if (model.get(s).sequenceNum() != 0) {
                                if (model.get(s).connectable(sq)) {
                                    nFound = 1;
                                    found = model.get(s);
                                }
                            }
