...
public class MatrixUtils {
    ...
    public static double threeMin(double a, double b, double c) {
    ...
    public static double[][] accumulateVertical(double[][] m) {
        ...
        for(int j=1;j<height;j++){
            for(int k=0;k<width;k++){
                if(k==0){
                    out[j][k]=m[j][k]+Math.min(get(out,j-1,k),get(out,j-1,k+1));
                }
                if(k==width-1){
                    out[j][k]=m[j][k]+Math.min(get(out,j-1,k),get(out,j-1,k-1));
                }
                else {
                    out[j][k] = m[j][k] + threeMin(get(out, j - 1, k), get(out, j - 1, k - 1), get(out, j - 1, k + 1));
                }
            };
        }
        return out;
    ...
    public static double[][] transpose(double[][] original){
        double[][] res = new double[original[0].length][original.length];
    ...
    public static double[][] accumulate(double[][] m, Orientation orientation) {
        if(orientation==Orientation.VERTICAL){
            ...
            double[][] t1 = transpose(m);
            double[][] t2 = accumulateVertical(t1);
            double[][] t3 = transpose(t2);
            return t3;
        }
    }
