...
final class Square {
    ...
    /**
     * Return if I am at corners.
     * @param sq sq.
     */
    static boolean isCorner(Square sq) {
        if ((sq.col() == 0 && sq.row() == 8)
            || (sq.col() == 8 && sq.row() == 8)
            || (sq.col() == 0 && sq.row() == 0)
            || (sq.col() == 8 || sq.row() == 0)) {
            return true;
    ...
    /**
     * Return 1-norm distance between two squares.
     * @param sq1 sq1
     * @param sq2 sq2
     */
    static int distance(Square sq1, Square sq2) {
        return Math.abs(sq1.row() - sq2.row())
            + Math.abs(sq1.col() - sq2.col());
    }

    /**
     * Change my selected state.
     */
    void clickMe() {
        selected = !selected;
    }
